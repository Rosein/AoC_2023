...
#include <sstream>
MapRange evaluate_value_of_mapping(MapRange input_to_evaluate, std::vector<MapSubcharacterisctic> map_to_evaluate)
{
...
std::vector<MapSubcharacterisctic> prepare_map_subcharacteristic(std::string raw_map)
{
    ...
    while(ss >> std::get<kDestinationStart>(candi)
             >> std::get<kSourceStart>(candi)
             >> std::get<kRangeLenght>(candi))
    {
