...
void calcucale_one_step_difference(Report& report, const int depth)
{
    ...
    assert(static_cast<int>(report.size()) > depth);
...
void calcucale_final_values(Report& report)
{
    ...
    for(int i = 1; static_cast<std::size_t>(i) < report.size() && !are_zeroes_reached; ++i)
    {
        calcucale_one_step_difference(report, i);
        are_zeroes_reached =
            std::all_of(std::begin(report), std::prev(std::end(report), i),
...
void calculate_extrapolated_values(Report& report)
{
    ...
    for(int i = 1; static_cast<std::size_t>(i) < report.size(); i++)
    {
